...
def input_fn(texts, labels, tokenizer, batch_size, mode):
    # Transform text/words to sequence of integers
    x = tokenizer.texts_to_sequences(texts)

    # Fix sequence length to max value. Sequences shorter than the length are
    # padded in the beginning and sequences longer are truncated
    # at the beginning.
    # This allows to feed data into our model in batches
...
def keras_estimator(model_dir,
    ...
    model.add(Conv1D(filters=filters * 2,
                              ...
                              bias_initializer='random_uniform',
                              padding='same'))
    model.add(GlobalAveragePooling1D())  # pool size = size of the input, way of flattening the output of our CNN 
    model.add(Dropout(rate=dropout_rate))
...
def train_and_evaluate(output_dir, hparams):
    ...
    # Save token dictionary (vocabulary mapping) to disk to use during prediction time
